...
module.exports = class VisualService {
    ...
    genData(text, color) {
        return {
            text: text,
            color: color || "",
            back: this.backround + this.textstyle
        }
    }
    setRow(row, text, color, layer) {
        text = text.split("")
        for (var i = 0; i < text.length; i++) {
            this.set(i, row, this.genData(text[i], color), layer)
    ...
    update() {
        ...
        for (var b = 0; b < this.height - 1; b++) {
            if (!this.visible[b]) {
                ...
                process.stdout.write('\x1b[0m' + this.visible[b] + EOL)
            }
        }
    }
